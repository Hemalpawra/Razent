...
export async function completeACPCheckoutSession(
  ...
  await executeAgentCheckout({
    order: {
      ...
      shipping_address: {
        full_name: buyerName,
        phone: session.fulfillment_details?.phone_number || "",
        email: buyerEmail,
        line1: session.fulfillment_details?.address?.line_one || "",
        city: session.fulfillment_details?.address?.city || "",
        state: session.fulfillment_details?.address?.state || "",
        pincode: session.fulfillment_details?.address?.postal_code || "",
        country: "India",
      },
      via_ai: true,
