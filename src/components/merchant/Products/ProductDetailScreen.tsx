...
export default function ProductDetailScreen() {
  ...
  const product = drawerProductId ? mockProducts.find((p) => p.id === drawerProductId) ?? null : null

  const sku = product ? getSku(product) : ""
  const updated = product ? new Date(product.updated_at).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" }) : ""
  const compareAt = product ? product.price_paise + Math.round(product.price_paise * 0.18) : 0
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-10 flex h-12 items-center gap-3 border-b bg-background/80 px-4 backdrop-blur">
        <Button variant="ghost" size="icon" onClick={handleBack} className="p-1">
      ...
      <div className="p-4 space-y-4">
        {!product ? (
          ...
          <>
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    ...
                    <Badge variant="secondary" className="rounded-full text-[11px]">{product.category}</Badge>
                  </div>
                </div>
              </div>
            </Card>
            ...
            <Card className="p-4">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">{formatPrice(product.price_paise)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Compare-at price</span>
                  <span className="font-medium text-muted-foreground line-through">{formatPrice(compareAt)}</span>
                </div>
              </div>
            </Card>
            ...
            {/* Stock */}
            <Card className="p-4">
              <h3 className="text-sm font-semibold text-foreground mb-3">Inventory</h3>
