import { Users, ShoppingCart, IndianRupee, MessageCircle, TrendingUp, Package, Lightbulb, AlertTriangle, Eye, ChevronDown, Download, Bot, User, Star, Send, XIcon, Sparkles } from "lucide-react"
...
const QUICK_CHIPS = ["Yes, proceed", "Show cheaper options", "Compare all 3", "Add to cart"]
...
export default function AIAgentScreen({ loading, error }: { loading?: boolean; error?: string | null }) {
  ...
  const selected = selectedId ? mockConversations.find((c) => c.id === selectedId) ?? null : null
    ...
    <div className="space-y-3 bg-muted/30 -m-6 p-6">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button className="h-9 rounded-lg" onClick={() => handleOpen(mockConversations[0].id)}>Test AI</Button>
      ...
      {!splitOpen ? (
        ...
        <div className="grid gap-3 lg:grid-cols-[1fr_380px]">
          ...
          <div className="min-w-0 xl:h-[calc(100vh-140px)] xl:overflow-auto xl:pr-1">
            <LiveConversationsCard onOpen={handleOpen} selectedId={selectedId} splitMode />
          </div>
          ...
          <Card className="flex flex-col overflow-hidden rounded-xl bg-card border shadow-sm xl:h-[calc(100vh-140px)] xl:sticky xl:top-3">
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-foreground truncate">ChatGPT Assistant</span>
                      <Badge variant="success" className="rounded-full px-2 py-0 text-[11px] shrink-0">Active</Badge>
                    </div>
                    <div className="text-[11px] leading-4 text-muted-foreground truncate">{selected?.customer_name ?? "Ananya Rao"} · {selected ? sourceLabel(selected.type) : "AI Assistant"} · {selected ? statusLabel[selected.status] : "Active"}</div>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <Badge variant="secondary" className="rounded-full text-[11px] hidden sm:inline-flex">AI Assistant</Badge>
                  <Button variant="ghost" size="icon-sm" onClick={handleCloseSplit} aria-label="Close assistant"><XIcon className="size-4" /></Button>
                </div>
              </div>
              <div className="mt-2 flex items-center gap-1.5 text-[11px] text-muted-foreground">
                <span className="inline-flex items-center gap-1"><Sparkles className="size-3" /> Live assistant workspace</span>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 bg-muted/20">
              ...
              <div className="flex flex-wrap gap-2 pl-1">
                {QUICK_CHIPS.map((chip) => (
                  <Button key={chip} variant="outline" size="sm" className="h-7 rounded-full bg-card text-xs" onClick={() => setInputValue(chip)}>
                    {chip}
                  </Button>
                ))}
              </div>
            ...
            <div className="shrink-0 border-t bg-card px-3 py-3">
              <div className="flex items-center gap-2">
                ...
                />
                <Button size="icon" className="size-9 shrink-0 rounded-full" onClick={() => setInputValue("")} aria-label="Send">
                  <Send className="size-4" />
                </Button>
              </div>
              <div className="mt-2 flex items-center justify-between text-[11px] text-muted-foreground">
                <span className="inline-flex items-center gap-1"><Bot className="size-3" /> AI drafting · press Enter to send</span>
                <Button variant="ghost" size="sm" className="h-6 rounded-full text-[11px]" onClick={handleCloseSplit}>Close workspace</Button>
              </div>
            </div>
          </Card>
        </div>
      ...
      <ConversationDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} conversation={selected} />
    </div>
  )
}

function LiveConversationsCard({ onOpen, selectedId, splitMode }: { onOpen: (id: string) => void; selectedId?: string | null; splitMode?: boolean }) {
  return (
    <Card className="overflow-hidden rounded-xl bg-card py-0 flex flex-col">
        ...
        <Table>
          ...
          <TableBody>
            {mockConversations.slice(0, 5).map((c) => (
              <TableRow key={c.id} className={"hover:bg-muted/20 " + (selectedId === c.id && splitMode ? "bg-primary/[0.04]" : "")}>
                ...
                <TableCell className="px-4 py-3 text-right">
                  <Button variant={selectedId === c.id && splitMode ? "default" : "outline"} size="sm" className="h-7 rounded-md" onClick={() => onOpen(c.id)}><Eye className="size-3.5" /> Open</Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
      ...
      <div className="flex items-center justify-between border-t bg-card px-4 py-3 text-xs text-muted-foreground"><span>Showing 5 of {mockConversations.length} conversations</span><Button variant="outline" size="sm" className="h-7 rounded-full bg-card" onClick={() => onOpen(mockConversations[0].id)}>Open latest</Button></div>
    </Card>
  )
}
