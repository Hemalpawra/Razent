import { Bot, Users, ShoppingCart, IndianRupee, MessageCircle, TrendingUp, Package, Lightbulb, AlertTriangle, Eye } from "lucide-react"
  ...
  if (loading) {
    return (
      <div className="space-y-4 bg-muted/30 -m-6 p-6">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-28 rounded-xl" />)}</div>
        <div className="grid gap-4 lg:grid-cols-3"><Skeleton className="h-64 lg:col-span-2 rounded-xl" /><Skeleton className="h-64 rounded-xl" /></div>
    ...
    return (
      <div className="bg-muted/30 -m-6 p-6">
        ...
        <Card className="mt-6 rounded-xl bg-card p-10 text-center">
          <div className="mx-auto flex size-12 items-center justify-center rounded-full bg-primary/10 text-primary"><Bot className="size-6" /></div>
  ...
  return (
    <div className="space-y-4 bg-muted/30 -m-6 p-6">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="font-heading text-[32px] font-semibold leading-[38px] tracking-tight text-foreground">AI Agent</h1>
            <Badge variant="success" className="rounded-full gap-1.5 px-2.5 py-0 text-xs"><span className="size-1.5 rounded-full bg-emerald-500" /> Online</Badge>
          </div>
          <p className="mt-1 text-sm leading-6 text-muted-foreground">Monitor how your AI helps customers and drives more sales.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Button variant="outline" className="h-9 rounded-lg bg-card">Test AI</Button>
          <Button variant="outline" className="h-9 rounded-lg bg-card hidden sm:inline-flex"><Eye className="size-4" />View Audit Trail</Button>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard icon={<Bot className="size-4" />} label="AI Status" value="Online" sub="Responding in ~1.2s avg" valueIsText />
      ...
      {/* Row: Performance + Needs Attention */}
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="rounded-xl bg-card lg:col-span-2">
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-xs">Today&apos;s impact at a glance</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
              <Metric label="Customers helped" value="42" sub="today" />
              <Metric label="Orders created" value={String(ordersToday || 18)} sub="via AI" />
              <Metric label="Revenue generated" value={formatPrice(revenueToday || 82500_00)} sub="today" />
              <Metric label="Conversion rate" value="24.5%" sub="↑ 5.3% vs yesterday" highlight />
            </div>
            <Separator className="my-4" />
            <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
              <span className="inline-flex items-center gap-1.5"><span className="size-2 rounded-full bg-primary" />AI answered 98% without handoff</span>
              <span className="inline-flex items-center gap-1.5"><span className="size-2 rounded-full bg-emerald-500" />Avg response 1.2s</span>
            </div>
          </CardContent>
        </Card>
          ...
          <Table>
            <TableHeader className="bg-muted/40">
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead className="h-10 px-3 text-xs font-semibold text-foreground">Type</TableHead>
              ...
              {mockConversations.slice(0, 5).map((c) => (
                <TableRow key={c.id} className="hover:bg-muted/20">
                  ...
                  <TableCell className="px-3 py-3">
                    <Badge variant="outline" className="rounded-full text-[11px]">{c.type === "human_customer" ? "Human Customer" : "Agent to Agent"}</Badge>
                  </TableCell>
                  <TableCell className="px-3 py-3">
...
function KpiCard({ icon, label, value, sub, valueIsAmount, valueIsText }: { icon: React.ReactNode; label: string; value: string; sub: string; valueIsAmount?: boolean; valueIsText?: boolean }) {
  return (
    <Card className="rounded-xl bg-card p-5 shadow-sm py-5">
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className={valueIsAmount ? "mt-0.5 text-lg font-semibold leading-6 text-foreground" : valueIsText ? "mt-0.5 text-lg font-semibold leading-6 text-foreground" : "mt-0.5 font-heading text-[22px] font-semibold leading-7 text-foreground"}>{value}</div>
...
function Metric({ label, value, sub, highlight }: { label: string; value: string; sub: string; highlight?: boolean }) {
  return (
    <div className="rounded-lg border bg-card p-3">
      ...
      <div className={"mt-0.5 text-[10px] " + (highlight ? "text-emerald-600 dark:text-emerald-400 font-medium" : "text-muted-foreground")}>{sub}</div>
    </div>
  )
}
